...
2. Go to your repository at [collective/ploneutfpr](https://github.com/collective/ploneutfpr).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `ploneutfpr.example.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | ploneutfpr.example.com | The hostname or IP address of your Docker Swarm manager. |
...
| Name     | Value |
|----------|-------|
| LIVE_ENV | ploneutfpr.example.com |
...
- [Backend Tests Workflow](https://github.com/collective/ploneutfpr/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/collective/ploneutfpr/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`collective/ploneutfpr-backend`) and Frontend (`collective/ploneutfpr-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of ploneutfpr.example.com](https://github.com/collective/ploneutfpr/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/ploneutfpr.example.com.yml` stack.
